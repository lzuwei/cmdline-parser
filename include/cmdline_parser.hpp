...
public:
    ...
    int parse_arguments(int argc, char *argv[], int cur_pos) override {
        m_isset = true;
        return cur_pos;
    }
};
...
public:
    arg() {}
    arg(const std::string &description) : m_description(description) {}
...
public:
    ...
    explicit positional_arg(const std::string &description)
        :
        arg(description) {}

    void parse_arguments(const std::string &arguments) override {
        try {
